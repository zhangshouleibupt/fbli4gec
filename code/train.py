...
class BaseTrainner(object):
	...
	def update_parameters_step(self,model,optimizer,dataset,cur_epoch=0,every_barch_to_save=6):
		...
		for batch_num,batch in enumerate(train_dataloader):
			...
			for i in range(1,self.max_len-1):
				...
				loss = self.criterion(output,output[i+1])
			...
			if (batch_num + 1) % every_barch_to_save == 0:
...
class BackBoostTrainner(BaseTrainner):
	...
	def train(self,criterion):
 	...
 	def _generate_one_epoch_disfulent_set(self,back_boost_disfluent_dataset,size=0.9):
 		...
 		choosed_pairs = [(random.choice(back_boost_disfluent_dataset[index][1]),back_boost_disfluent_dataset[index][0])
